...
const levelConfig: Record<string, { size: number; color: string }> = {
  Experienced: { size: 64, color: "bg-[var(--color-warm)]" },
  Intermediate: { size: 48, color: "bg-[var(--color-text-muted)]/50" },
  Basic: { size: 36, color: "bg-[var(--color-text-muted)]/25" },
};
...
function getReposForSkill(skillName: string, projects: PortfolioData["projects"]) {
  ...
  return projects.filter((p) =>
    p.tags.some((t) => tags.some((tag) => t.toLowerCase().includes(tag.toLowerCase())))
  );
}
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          width: skill.level === 'Experienced' ? undefined : undefined,
          height: skill.level === 'Experienced' ? undefined : undefined,
        }}
      />
...
function SkillRow({
  ...
  return (
    <div className="mb-8 sm:mb-10 last:mb-0">
      ...
      {/* Responsive grid: 2 cols mobile, 4 cols sm, 6 cols md, 8 cols lg */}
